...
uint8_t reverseBits(uint8_t n) {
    uint8_t reversed = 0;
    for (int i = 0; i < 8; i++) {
        reversed <<= 1;       // Desplaza los bits de reversed a la izquierda
        reversed |= (n & 1);  // Pone el bit menos significativo de n en el bit menos significativo de reversed
        n >>= 1;              // Desplaza los bits de n a la derecha
    }
    return reversed;
}
